        ...
        <div className={`relative ${isVisible ? "animate-fade-up stagger-3" : "opacity-0"}`}>
          <div className="flex gap-6 overflow-hidden">
            {reviews.slice(current, current + visible).map((r) => (
              <div key={r.name} className="flex-1 min-w-0 bg-background rounded-lg p-6 text-center transition-all duration-300 hover:shadow-md">
          ...
          <button onClick={() => setCurrent(Math.max(0, current - 1))} disabled={current === 0} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 w-10 h-10 rounded-full bg-card shadow-md flex items-center justify-center hover:bg-secondary transition disabled:opacity-30">
            <ChevronLeft className="w-5 h-5" />
          </button>
          <button onClick={() => setCurrent(Math.min(max, current + 1))} disabled={current >= max} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 w-10 h-10 rounded-full bg-card shadow-md flex items-center justify-center hover:bg-secondary transition disabled:opacity-30">
